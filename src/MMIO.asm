...
la $s1, buffer # Armazena em $s1 o endereço base do array do Buffer
add $t4, $s1, $zero # Armazena em $t4 o indice zero do array buffer
addi $t0, $zero, '\n' #Armazena em $t0 o valor ascii do \n

li $s0, 0xFFFF0000 # $s0 Recebe o endereço do Receiver Control, que sera usado como base para acessar os outros registradores

espera_Receiver_disponivel: # (0xFFFF0000, último biy == _0). Enquanto o último bit desse registrador for diferente de zero, ou seja não houver tecla digitada, volte no loop e esteja sempre disponível para receber, abordagem polling   
...
espera_Transmitter_disponivel: # (0xFFFF0008, ultimo bit == _0) Enquanto o ultimo bit for zero, ou seja, não estiver disponivel, tente novamente, até estar
...
#Checagem se o caracter inserido é um \n
beq $t2, $t0, enter_funcao # Checa se o caractere atual da String é um \n
